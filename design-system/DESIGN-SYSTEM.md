...
| `MOTION.md` | Spec de animação: princípio, regras, springs e catálogo por componente |
...
```html
<html data-theme="graph">      <!-- claro: creme quadriculado (default) -->
<html data-theme="blueprint">  <!-- escuro: azul de prancheta (dark mode) -->
```

> O site usa exatamente dois temas: graph como claro e blueprint como dark mode
...
Cada nível tem uma **assinatura visual própria** (tamanho + ornamento/fonte/cor), não só tamanho:

| Nível | Assinatura | Uso típico |
|---|---|---|
| `h1` | Display, gigante, tracking apertado (+ `.highlight` no hero) | título da página |
| `h2` | Display + sublinhado de marcador accent embutido | títulos de seção |
| `h3` | Display + traço de caneta accent na margem esquerda | subtítulos |
| `h4` | Gochi Hand (anotação manuscrita) | títulos de card/bloco |
| `h5` | Caps + letter-spacing, ink-soft (etiqueta) | overlines, labels |
| `h6` | Fira Code, ink-soft (metadado) | datas, paths, meta |

> Componentes podem resetar o ornamento quando ele vira ruído (ex.: `.project-card__title` remove o traço do h3).
